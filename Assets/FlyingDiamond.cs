...
public class FlyingDiamond : MonoBehaviour
{
    ...
    private IEnumerator GiveTarget(Transform target,int value, Action action=null)
    {
        ...
        foreach (var item in diamonds)
        {
            ...
            soundManager.PlayQuickSoundClip("coin");
        ...
        foreach (var item in diamonds)
        {
            ...
            item.DOMove(target.position, timeDifference-number*0.02f).OnComplete(()=> { item.gameObject.SetActive(false); soundManager.PlayQuickSoundClip("coin"); });
            number++;
        }
